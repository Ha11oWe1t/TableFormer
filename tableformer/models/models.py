...
class TableFormer(nn.Module):
    ...
    def forward(self, img, y_tags, y_bboxes, y_classes):
        ...
        enc_out = self.encoder(self.src_pe(x_f))  # (784, 1, 512)
        print(enc_out.shape)
        if y_tags is None:
            # TODO: greedy | beam
            y_src = torch.tensor([[self.start_idx], [self.td_a_idx]]).to(enc_out.device)
            terminal = False
            while not terminal:
                ...
                dec_out = self.decoder(
                    ...
                    memory=enc_out,
                    tgt_mask=generate_square_subsequent_mask(y_src.size(0)).to(enc_out.device),
                    tgt_key_padding_mask=padding_mask.permute(1, 0),
                    memory_mask=None
                )
                pred_tags = self.softmax(self.linear(dec_out))
                print(len(pred_tags))
                y_src = torch.argmax(pred_tags[:, :, :], dim=-1)
                #y_src = torch.cat([y_src, next_tag.unsqueeze(0)], dim=0)
                if y_src[-1] == self.end_idx:
                    terminal = True
        else:
            ...
            attn_out, _ = self.attention(dec_out_, linear_out, enc_out)  # q, k, v, 用linear_out代替enc_out
